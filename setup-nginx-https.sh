...
# Скрипт автоматической настройки Nginx и HTTPS для wayesports.org
# Запуск: sudo bash setup-nginx-https.sh

set -e

DOMAIN="wayesports.org"
...
sudo cp "$PROJECT_DIR/nginx/wayesports.org.conf" "/etc/nginx/sites-available/$DOMAIN"
